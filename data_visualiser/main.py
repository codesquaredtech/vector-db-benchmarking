...
def retrieve_benchmarker_files(type: str):
    file_paths = [os.path.join(RESULTS_FOLDER, f) for f in os.listdir(RESULTS_FOLDER) if type in f and f.endswith(".csv")]
    file_paths.sort()
    return file_paths
...
def plot_metrics_scatterplot(
    ...
    """
    Create a single scatterplot with a specified metric for all databases, using different colors for each database.
    ...
    for i, file_path in enumerate(search_file_paths):
        ...
        ax.plot(df_valid["iteration"], df_valid[metric], color=colors[i], linewidth=2, alpha=0.8)

    ax.set_xlabel("Iteration")
    ax.set_ylabel(ylabel if ylabel else metric)
    ax.set_title(title if title else f"{metric.capitalize()} Across Iterations for All Databases")
    ...
    if file_name is None:
        file_name = f"{metric}_combined_scatterplot.jpg"
    plot_path = os.path.join(PLOTS_FOLDER, file_name)

    fig.savefig(plot_path, dpi=300)

    logger.info(f"Saved combined scatter plot: {plot_path}")
...
def plot_metrics_barplot(
    ...
    for file_path in search_file_paths:
        ...
        for metric in metrics:
            logger.info(f"full metric value {df.loc[df["iteration"] == metric, column_name].values[0]}")
